...
export default function Sidebar() {
  ...
  const { restaurantDescription, restaurantName } = useSelector(
  ...
  return (
    <div style={{ zIndex: "999" }}>
      ...
      <div
        ...
        className={`bg-black bg-opacity-20 w-lvw h-lvh ${
          open ? "absolute" : "hidden"
        } top-0 left-0`}
      ></div>
      <div
        style={{ width: "100%", position: "fixed", top: "0%" }}
        className={`absolute bg-white h-lvh w-2/3 ${
          open ? "left-0" : "-left-full"
        } top-0 transition-all ease-in-out duration-300 p-2 flex flex-col justify-between`}
      >
        <IconX
          onClick={() => setOpen(false)}
