...
import { Slider } from "../AMT_Package";
export default function SearchBar() {
  const { restaurantName, searchHere } = useSelector(
    (state: RootState) => state.translation.words
  );
  ...
  return (
    ...
    >
      <div
        className={`${
          lang == "en" ? "font-primary" : "font-secondary"
        } text-primary`}
      >
        <p>{restaurantName}</p>
      </div>
      <div className="flex flex-col gap-4 w-full overflow-hidden">
        ...
        <div className="w-full overflow-x-scroll scroll-bar">
          <div
            ...
            className="flex gap-2 overflow-scroll w-fit"
          >
            <Slider
            ...
            >
              {data.map(({ image, link, name }) => (
                <a
                  href={`#${name}`}
                  key={link}
                  className="w-32 h-40 overflow-hidden flex flex-col gap-1 text-primary"
                >
                  <div
                    style={{
                      backgroundImage: `url(${image})`,
                    }}
                    className="w-32 h-32 bg-cover bg-center border-primary border-solid border-2 rounded-lg"
                  ></div>
                  <p>{name}</p>
                </a>
              ))}
